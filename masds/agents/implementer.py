# -----------------------------
# File: devagents/agents/implementer.py
# -----------------------------
import os
import sys
import json
...
def implementation_agent(state: ProjectState) -> ProjectState:
    ...
    **RESPONSE FORMAT** (note: no trailing commas, all strings properly quoted/escaped):
    ...
    """
        ...
        response = C.LLM.invoke([HumanMessage(content=prompt_template)])
        ...
        C.LOGGER.info("[Implementer] Reasoning: %s", response["reasoning"])
        print("==================================")
        print(json.dumps(response))
        print("==================================")

        execute_implementation("\n".join(response["implementation"]))
        
        prev_impl.append({
            "task_description": description,
            "task_implementation": response["implementation"]
        })

    result = copy.deepcopy(state)

    result["implementer"] = response
